...
class ICPagePosts {
    ...
    protected function paginate_links( $posts ){
        ...
        /* 
         * prepend a ? for params in the querystring if get_permalink didn't 
         * add it already ( case: default permalink ), otherwise use &
         */
        $q_amp = strpos( $page_url, "?" ) ? '&' : "?";
        
        // HTML string inserted for the PREVIOUS link
        $prev = ( $curr_page && $curr_page > 1 ) ? 
            '<li><a href="' . $page_url . $q_amp . 'page=' . ( $curr_page-1 ) .
                '">' . $this->args['label_previous'] . '</a></li>' : 
            '';
        // HTML string inserted for the NEXT link
        $next = ( $curr_page && $curr_page < $total_pages ) ? 
            '<li><a href="' . $page_url . $q_amp . 'page=' . ( $curr_page+1 ) . 
                '">' . $this->args['label_next'] . '</a></li>' : 
            '';
